...
jQuery(
    function ($) {
        ...
        $(document).on(
            'click', '.shortcake-insert-media-modal', function (event) {
                if (!modalFrame) {
                    modalFrame = wp.media(
                        {
                            multiple: false
        ...
        if ('undefined' !== typeof (wp.shortcake)) {
            wp.shortcake.hooks.addAction(
                'shortcode-ui.render_edit', function (shortcodeModel) {
            ...
            wp.shortcake.hooks.addAction(
                'shortcode-ui.render_new', function () {
                    richText.loaded = richText.load(richTextSelector);
                }
            );

            wp.shortcake.hooks.addAction(
                'shortcode-ui.render_destroy', function () {
                    richText.unload(richTextSelector);
                }
            );

            wp.shortcake.hooks.addAction(
                'shortcode-ui.render_closed', function () {
                    richText.unload(richTextSelector);
                }
            );
        }
